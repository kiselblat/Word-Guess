...
document.onkeyup = function(event) {
  ...
  if (outcome === "ongoing") {
    ...
    if (remainingLetters === 0) {
      ...
      ;
      outcome = "win";
      console.log(`Congratulations. ${endgameMessage(outcome)} Press space to play again.`);
    // user loses when they run out of guesses
    } else if (remainingTurns === 0) {
      LOSSES++;
      
      outcome = "loss";
      console.log(`The machine has won. You were trying to guess ${word}. ${endgameMessage(outcome)} Press space to play again.`);
    }
